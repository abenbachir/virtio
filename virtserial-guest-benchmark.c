...
static int send_bytestream_tsc(int val, unsigned long* stream_latency,
    unsigned long long* read_bytes)
{
    ...
    *stream_latency = elapsed_nsec(ts_start, ts_end);
...
int main(int argc, char *argv[])
{
    ...
    unsigned int i;
    int ret, cfd;
    unsigned long stream_latency;
    long double througthput;
    unsigned long long read_bytes;
...
back_to_open:
    ...
    while (1) {
        ...
        //printf("CMD: key=%s, value=%d\n", getKEY(gpkt.key), gpkt.value);
        switch(gpkt.key) {
        case KEY_OPEN:
        ...
        case KEY_GUEST_BYTESTREAM:
            ...
            //fseek(g_bigfile_fd, 0L, SEEK_END);
            //unsigned long file_size = ftell(g_bigfile_fd);
            //rewind(g_bigfile_fd);

            ret = send_bytestream_tsc(gpkt.value, &stream_latency, &read_bytes);
            send_report(cfd, ret);

            througthput = read_bytes*8/(stream_latency/1E9);
            //printf("Transfered [%.2f MB], buff_length=[%d B], througthput=[%0.2Lf Gbits/s]\n",
            printf("%d,%0.2Lf,%.2f\n",
                g_length, (long double)througthput/1E9, bytes_to_mb(read_bytes));
            break;
        case KEY_GUEST_CSUM:
